...
// Défis variés et intéressants pour toutes les catégories et difficultés
const challenges = [
    // ========== CATÉGORIE: TECHNIQUE ==========
    ...
    // Collaboration - Expert
    {
        ...
        difficulty: "expert",
        startDate: new Date(),
        endDate: new Date(Date.now() + 14 * 24 * 60 * 60 * 1000), // 2 semaines
        isActive: true,
        maxTeams: 4,
        requirements: {
            teams: "min_3",
    ...
    
    {
        ...
        difficulty: "difficile",
        startDate: new Date(),
        endDate: new Date(Date.now() + 2 * 24 * 60 * 60 * 1000), // 48h
        isActive: true,
        maxTeams: null,
        requirements: {
            duration: "48h",
...
async function addChallenges() {
    try {
        ...
        // Supprimer les défis existants (optionnel - commentez si vous voulez garder les anciens)
        // await Challenge.destroy({ where: {}, truncate: true });
        // console.log("🗑️  Anciens défis supprimés");
        ...
        for (const challengeData of challenges) {
            try {
                // Vérifier si le défi existe déjà (par titre)
